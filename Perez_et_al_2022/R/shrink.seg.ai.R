...
shrink.seg.ai<-function(chr.seg){
  ...
  if(nrow(chr.seg) > 1){
    ...
    for(j in 2:nrow(new.chr)){
      ...
      if(seg_test){
	  #If those are equal, then save the rownumber of the previous segment
          seg.class <- c(seg.class, seg.class[j-1])
        }else{
    seg.class <- c(seg.class, seg.class[j-1]+1)
  }
    }
    #For each of the unique rownumbers (consecutive segments, initial row), select the maximum End_position (final position of the continuous segment)
